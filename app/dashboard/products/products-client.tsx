...
import { Plus, Package, TrendingUp, AlertCircle, Search, ChevronLeft, ChevronRight, Loader2, ExternalLink } from "lucide-react";
...
export function ProductsClient({ initialProducts, initialOrgSlug }: ProductsClientProps) {
    // Estado inicializado con datos del servidor — sin spinner en primera carga
    const [products, setProducts] = useState<(Product & { categories: { name: string } | null })[]>(initialProducts);
    const [loading, setLoading] = useState(false);
    const [dialogOpen, setDialogOpen] = useState(false);
    const [selectedProduct, setSelectedProduct] = useState<Product | undefined>();
    const [searchTerm, setSearchTerm] = useState("");
    const [orgSlug] = useState<string | null>(initialOrgSlug);
    ...
    // fetchProducts solo se llama DESPUÉS de mutaciones para refrescar
        ...
        <div className="space-y-8 pb-10">
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                ...
                <Card className="premium-shadow border-none bg-white dark:bg-slate-900 overflow-hidden group">
                    ...
                    <CardContent>
                        <div className="text-2xl font-black tracking-tight leading-none text-gray-900 dark:text-white">
                            {`$${kpis.inventoryValue.toLocaleString("es-CL")}`}
                        </div>
